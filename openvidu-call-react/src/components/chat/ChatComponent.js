...
import AvatarStatic from './avatar.png';
    ...
    componentDidMount() {
        this.props.user.getStreamManager().stream.session.on('signal:chat', (event) => {
            ...
            setTimeout(() => {
                ...
                //Agregar una imagen estatica para los usuarios                
                
                var avatarEstatico=new Image();                
                avatarEstatico.src = AvatarStatic;
                avatarEstatico.onload = function(){
                    avatar.drawImage(avatarEstatico,5, 0, 50, 50)
                }
                


                //Esta es la linea que estaba antes de agregar el avatar estatico
                //avatar.drawImage(video, 200, 120, 285, 285, 0, 0, 60, 60);// Hace una imagen del contenido de la pantalla                                


                this.props.messageReceived(); //La comentamos pues daba un pantallazo que se ve mal
            }, 50);
